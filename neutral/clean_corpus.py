...
def record(
    ...
    return {
        ...
        'version': None,
        'transformations': ['utf8_decode_replace', 'newline_normalization', 'whitespace_normalization'],
        'quality_flags': [],
        'text': text,
    }, None
...
def main():
    parser = argparse.ArgumentParser(
        description='Build a provenance-preserving JSONL corpus from approved local files.'
    )
    ...
    accepted = rejected = 0
    with args.output.open('w', encoding='utf-8') as out, args.rejected.open(
        'w', encoding='utf-8'
    ) as bad:
        for path in sorted(p for p in args.input.rglob('*') if p.is_file()):
            ...
            if item is None:
                bad.write(
                    json.dumps({'path': str(path), 'reason': reason}, ensure_ascii=False)
                    + '\n'
                )
                rejected += 1
                continue
    ...
    print(
        json.dumps(
            {'accepted': accepted, 'rejected': rejected, 'output': str(args.output)},
            ensure_ascii=False,
        )
    )
