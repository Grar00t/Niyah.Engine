...
def load_model(model_ref: str):
    ...
    if model_path.is_dir() and (model_path / "adapter_config.json").exists():
        return AutoPeftModelForCausalLM.from_pretrained(model_ref, **common)

    return AutoModelForCausalLM.from_pretrained(model_ref, **common)


def build_prompt(tokenizer, prompt: str, *, requires_chat_template: bool) -> str:
    ...
    if requires_chat_template:
        raise RuntimeError(
            "this model requires its chat template; tokenizer.chat_template is missing"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate text with a local model or PEFT adapter"
    )
    ...
    rendered_prompt = build_prompt(
        tokenizer,
        args.prompt,
        requires_chat_template=profile.requires_chat_template,
    )
