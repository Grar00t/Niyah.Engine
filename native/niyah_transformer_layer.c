...
size_t niyah_transformer_scratch_floats(const NiyahModelConfig* config)
{
    ...
    /* norm + q/tmp + k + v + attn + gate + up + scores */
    return dim + dim + kv_dim + kv_dim + dim + ff + ff + ctx;
}
    ...
    float* scratch)
{
    ...
    niyah_matvec(q, w->wo, attn, dim, dim);
    niyah_add_inplace(hidden, q, dim);
    ...
    niyah_matvec(q, w->ffn_down, up, dim, ff);
    niyah_add_inplace(hidden, q, dim);
