    ...
    size_t dlogits_count)
{
    ...
    supervised_count = token_count - loss_start;

    for (t = loss_start; t < token_count; ++t) {
        const float *row =
            logits + t * vocab_size;
    ...
    *out_loss =
    ...
    return isfinite(*out_loss)
        ? NIYAH_OK
        : NIYAH_ERR_INVALID_ARGUMENT;
}

NiyahStatus niyah_cross_entropy_loss(
    const float *logits,
    const uint32_t *targets,
    size_t token_count,
    size_t vocab_size,
    ...
    size_t dlogits_count)
{
    return niyah_cross_entropy_loss_masked(
        logits,
        targets,
        token_count,
        vocab_size,
