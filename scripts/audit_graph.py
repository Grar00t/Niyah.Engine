...
REQUIRED_EVIDENCE_EDGES = {
    "contradicts", "conflicts_with", "supersedes", "causes", "mitigates"
}
...
def audit(graph: dict) -> dict:
    ...
    unknown_node_types = sorted(
        {n.get("type") for n in nodes if n.get("type") not in ALLOWED_NODE_TYPES}
    )
    unknown_edge_types = sorted(
        {e.get("type") for e in edges if e.get("type") not in ALLOWED_EDGE_TYPES}
    )
    orphan_edges = [
        e.get("id")
        for e in edges
    ...
    unsupported_asserted_nodes = [
        n.get("id")
        for n in nodes
        if n.get("status") == "asserted"
        and not any(ev in evidence_ids for ev in n.get("provenance", []))
    ]
    missing_required_evidence = [
        ...
        and not any(
            ev in evidence_ids
            for ev in (e.get("provenance", []) + e.get("evidence", []))
        )
    ]

    valid_shape_nodes = all(
        bool(n.get("id")) and bool(n.get("type")) and bool(n.get("label"))
        for n in nodes
    )
    valid_shape_edges = all(
        bool(e.get("id"))
        and bool(e.get("source"))
        and bool(e.get("target"))
        and bool(e.get("type"))
        for e in edges
    )
    provenance_objects = len(nodes) + len(edges)
    provenance_bound = sum(bool(n.get("provenance")) for n in nodes) + sum(
        bool(e.get("provenance")) for e in edges
    )
    provenance_coverage = (
        provenance_bound / provenance_objects if provenance_objects else 1.0
    )
    completeness_score = (
        sum(
            bool(n.get("id")) and bool(n.get("type")) and bool(n.get("label"))
    ...
    conflicting = sum(
        ...
        and e.get("type") in {"contradicts", "conflicts_with"}
    )
    contradiction_index = conflicting / validated if validated else 0.0

    schema_compliance = bool(
        valid_shape_nodes
        and valid_shape_edges
        and not unknown_node_types
        and not unknown_edge_types
    )
    referential_integrity = not orphan_edges
    reasons = []
    if not schema_compliance:
        reasons.append("schema_compliance")
    if not referential_integrity:
        reasons.append("referential_integrity")
    if duplicate_ids:
        reasons.append("duplicate_ids")
    if unsupported_asserted_nodes:
        reasons.append("unsupported_asserted_nodes")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit a Niyah knowledge graph")
