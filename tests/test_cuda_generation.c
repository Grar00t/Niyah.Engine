...
int main(void)
{
    ...
    /*
     * Seeded stochastic parity uses the same model and prompt as the greedy
     * case. Exact token and final externally visible generation state must
     * match between CPU and CUDA for the shared sampler seed.
     */
    ...
    config.sampler.seed = UINT64_C(0x5eed5eed12345678);
